    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-bold text-[#14532d] text-lg">Role & Hak Akses</h3>
            <p className="text-xs text-gray-500">Atur permission View dan Edit per modul untuk setiap role</p>
          </div>
        </div>
        <button
          onClick={() => setShowAdd(true)}
          className="flex items-center gap-2 px-5 py-2.5 rounded-xl font-semibold text-sm text-white bg-[#14532d] hover:bg-[#052e16] transition-all shadow-md active:scale-95"
        >
      ...
      {roles.map((role, ri) => (
        <div key={role.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          ...
          <div className="px-5 py-4 bg-gray-50/80 border-b border-gray-100 flex items-center justify-between">
            <div className="flex items-center gap-3">
              <span className="font-black text-[#14532d] uppercase tracking-wide text-sm">{role.name}</span>
              {role.is_system ? (
            ...
            <table className="w-full text-left">
              ...
              <tbody className="text-sm">
                {modules.map(mod => {
                  ...
                  return (
                    <tr key={mod} className="border-b border-gray-50 hover:bg-emerald-50/20 transition-colors">
      ...
      {showAdd && (
        <div className="fixed inset-0 flex items-center justify-center z-50 backdrop-blur-sm bg-black/40">
          <div className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-2xl">
            <h3 className="text-lg font-black text-[#14532d] mb-4">Tambah Role Baru</h3>
            <input
              type="text"
              value={newRoleName}
              onChange={e => setNewRoleName(e.target.value)}
              placeholder="Nama role (misal: supervisor)"
              className="w-full px-4 py-3 rounded-xl text-sm border border-gray-200 bg-gray-50 focus:outline-none focus:border-[#14532d] mb-4"
            />
            ...
            <div className="flex gap-3">
              ...
              <button onClick={addRole} disabled={addLoading} className="flex-1 py-3 rounded-xl font-bold text-sm text-white bg-[#14532d] disabled:opacity-50">
