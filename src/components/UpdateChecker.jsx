...
export const CURRENT_APP_VERSION = '1.0.0'; // Sesuaikan ini setiap kali build APK / OTA baru
...
export default function UpdateChecker() {
  ...
  if (isUpdating) {
    return (
      <div className="fixed inset-0 z-[9999] bg-black/70 backdrop-blur-sm flex flex-col items-center justify-center p-6 text-center text-white">
        <div className="bg-[#14532d] p-8 rounded-2xl shadow-2xl max-w-sm w-full border border-white/20 flex flex-col items-center">
  ...
  if (updateInfo.force_update) {
    return (
      <div className="fixed inset-0 z-[9999] bg-[#F9F5F0] flex flex-col items-center justify-center p-6 text-center">
        <div className="bg-white p-8 rounded-2xl shadow-xl max-w-sm w-full border-t-4 border-[#14532d]">
          <h2 className="text-2xl font-black text-[#14532d] mb-2">Update Wajib</h2>
          <p className="text-gray-600 mb-6 text-sm">
            {updateInfo.message || 'Versi aplikasi New Chapter Anda sudah kadaluwarsa. Silakan download versi terbaru untuk melanjutkan.'}
          </p>
          <p className="text-xs font-bold text-gray-500 mb-4">Versi Terbaru: {updateInfo.latest_version}</p>
          <button
            onClick={handleOtaUpdate}
            className="block w-full py-3 px-4 bg-[#14532d] hover:bg-[#4d3925] text-white font-bold rounded-lg shadow-md transition-all text-center"
          >
            Update & Restart
          </button>
        </div>
  ...
  return (
    <div className="fixed top-0 left-0 right-0 z-[9999] bg-[#14532d] text-white px-4 py-3 shadow-lg flex items-center justify-between">
      ...
      <div className="flex items-center gap-2">
        <button
          onClick={handleOtaUpdate}
          className="px-3 py-1.5 bg-white text-[#14532d] text-xs font-black rounded hover:bg-gray-100 transition-all whitespace-nowrap"
        >
          Update & Restart
        </button>
        <button
