...
export default function UserManage() {
  ...
  const handleShowPublicMenuQR = () => {
    ...
    setQrUrl(`https://menu.NewChapter.cloud`)
    setShowQR(true)
  }
  ...
  return (
    <MobileLayout activeMenu="User Manage">
      ...
      <div className="hidden lg:flex justify-between items-center px-6 xl:px-10 py-5 bg-white/80 backdrop-blur-md sticky top-0 z-10 border-b border-emerald-100/50 shadow-sm">
        ...
        <div className="flex items-center gap-4">
          <div className="text-right">
            ...
            <p className="text-xs uppercase" style={{ color: '#d4af37' }}>{user?.role || 'Admin'}</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
              ...
              {/* Stats, QR Code & Add Button */}
                <div className="rounded-2xl p-4 shadow-sm flex flex-col justify-center border relative overflow-hidden group bg-white border-[#EDE0CC]">
                  ...
                  <p className="text-3xl font-black text-[#14532d] relative z-10">{userList.length}</p>
                </div>
                <div className="rounded-2xl p-4 shadow-sm flex flex-col justify-center border relative overflow-hidden group bg-white border-[#EDE0CC]">
                  <div className="absolute -right-4 -bottom-4 w-16 h-16 bg-emerald-50 rounded-full blur-xl group-hover:scale-150 transition-transform"></div>
                ...
                {userCanEdit('user_manage') && (
                  <button
                    ...
                    className="rounded-2xl p-4 shadow-sm transition-all hover:-translate-y-1 hover:shadow-md flex flex-col items-center justify-center gap-1.5 relative overflow-hidden group text-white bg-[#14532d]"
                  >
                  ...
                  <table className="w-full text-left border-collapse">
                    ...
                    <tbody className="text-sm">
                      {userList.length === 0 ? (
                        ...
                        <tr key={u.id} className="border-b border-gray-50 hover:bg-emerald-50/30 transition-colors group">
                          <td className="px-8 py-5">
                            <div className="flex items-center gap-3">
                              <div className="w-10 h-10 rounded-full bg-gray-100 text-[#14532d] flex items-center justify-center font-bold text-lg shadow-sm">
                                {(u.nama || '?')[0].toUpperCase()}
                              </div>
                              <span className="font-bold text-[#14532d] text-base">{u.nama}</span>
                              {u.aktif === 0 && (
                          ...
                          <td className="px-8 py-5">
                            <span
                              ...
                              style={{
                                ...
                                color: roleColor[u.role]?.color || '#14532d',
                                ...
                                <>
                                  {u.is_logged_in ? (
                                    <button
                                      ...
                                      className="px-4 py-2 rounded-xl text-xs font-bold transition-all hover:opacity-80 shadow-sm border border-emerald-200 bg-emerald-50 text-emerald-600 hover:bg-emerald-100"
                                    >
                                      Reset Sesi
                                    </button>
                                  ) : null}
                                  <button
                                    ...
                                    className="px-4 py-2 rounded-xl text-xs font-bold transition-all hover:opacity-80 shadow-sm border border-emerald-200 bg-emerald-50 text-emerald-700 hover:bg-emerald-100"
                                  >
                                    Edit
                                  </button>
                                  <button
        ...
        <div className="fixed inset-0 flex items-center justify-center z-50 backdrop-blur-sm" style={{ backgroundColor: 'rgba(0,0,0,0.4)' }}>
          <div className="rounded-3xl p-8 w-full max-w-md shadow-2xl animate-in zoom-in-95 duration-200" style={{ backgroundColor: '#fff', border: '1px solid #EDE0CC' }}>
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-2xl font-black" style={{ color: '#14532d' }}>Tambah User Baru</h2>
            </div>
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-3 rounded-xl text-sm font-medium focus:outline-none focus:ring-2 focus:ring-emerald-500/20 transition-all"
                  style={{ backgroundColor: '#F9F5F0', color: '#14532d', border: '1px solid #EDE0CC' }}
                />
              </div>
              <div>
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full px-4 py-3 rounded-xl text-sm font-bold focus:outline-none focus:ring-2 focus:ring-emerald-500/20 transition-all"
                  style={{ backgroundColor: '#F9F5F0', color: '#14532d', border: '1px solid #EDE0CC' }}
                >
            ...
            <div className="flex gap-3 mt-8">
              ...
              <button
                ...
                style={{ backgroundColor: '#14532d', boxShadow: '0 8px 20px rgba(99, 73, 48, 0.2)' }}
              >
      ...
      {showEditRole && editTarget && (
        <div className="fixed inset-0 flex items-center justify-center z-50 backdrop-blur-sm" style={{ backgroundColor: 'rgba(0,0,0,0.4)' }}>
          <div className="rounded-3xl p-8 w-full max-w-md shadow-2xl animate-in zoom-in-95 duration-200" style={{ backgroundColor: '#fff', border: '1px solid #EDE0CC' }}>
            <h2 className="text-2xl font-black mb-2" style={{ color: '#14532d' }}>Edit User</h2>
            <div className="mb-6 p-4 rounded-2xl bg-emerald-50 border border-emerald-100 flex items-center gap-3">
               <div className="w-10 h-10 rounded-full bg-white text-[#14532d] flex items-center justify-center font-bold text-lg shadow-sm">
                 {(editTarget.nama || '?')[0].toUpperCase()}
               </div>
               <div>
                  <p className="text-sm font-bold" style={{ color: '#14532d' }}>{editTarget.nama}</p>
                  <p className="text-xs font-medium text-emerald-700">@{editTarget.username}</p>
               </div>
            </div>
            <div className="space-y-4">
              <div>
