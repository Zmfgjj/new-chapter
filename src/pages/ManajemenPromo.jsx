        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
          <div>
            <h1 className="text-2xl font-black text-[#442D1D] flex items-center gap-2">
              <Tag className="text-[#14532d]" size={28} /> Manajemen Promo
            </h1>
            <p className="text-[#d4af37] mt-1 text-sm font-medium">Atur diskon massal untuk menu jualan Anda.</p>
          </div>
            ...
            <button
              ...
              className={`px-6 py-2.5 rounded-full font-bold text-sm transition-all shadow-sm ${
                activeTab === 'normal' 
                  ...
                  : 'bg-white text-[#d4af37] border border-stone-200 hover:bg-stone-50'
              }`}
            >
          ...
          {activeTab !== 'campaigns' && (
            <div className="w-full md:w-64 relative">
              <input
                ...
                className="w-full pl-4 pr-4 py-2 bg-white border border-stone-200 rounded-full focus:outline-none focus:ring-2 focus:ring-[#14532d]/20 focus:border-[#14532d] text-sm text-[#442D1D]"
              />
            </div>
          )}
        </div>
                    ...
                    {canEdit('manajemen_promo') && (
                      <th className="py-3 px-4 w-12 text-center">
                        <button onClick={toggleSelectAll} className="text-stone-400 hover:text-[#14532d]">
                          {filteredList.length > 0 && filteredList.every(m => selectedIds.includes(m.id)) ? <CheckSquare size={20} className="text-[#14532d]" /> : <Square size={20} />}
                        </button>
                      </th>
                    )}
                ...
                <tbody className="divide-y divide-stone-100">
                  {filteredList.map(menu => {
                    ...
                    return (
                      <tr 
                        key={menu.id} 
                        className={`hover:bg-stone-50 transition-colors cursor-pointer ${isSelected ? 'bg-emerald-50/50' : ''}`}
                      ...
                      >
                        ...
                        <td className="py-3 px-4 text-right">
                          <span className={`text-sm font-bold ${activeTab === 'promo' ? 'text-stone-400 line-through' : 'text-[#14532d]'}`}>
