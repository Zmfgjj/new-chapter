...
export default function Login() {
  ...
  const handleLoginAttempt = async (force = false) => {
    ...
    try {
      ...
      const data = { user: mockUser, token: 'showcase-token' }
      
      await loginSuccess(data, password || '123')
      ...
      <div className="w-full max-w-sm mx-4 rounded-3xl shadow-xl p-10 flex flex-col items-center relative overflow-hidden" style={{ backgroundColor: '#E8D5B7' }}>
        ...
        {showForcePrompt && (
          <div className="absolute inset-0 z-10 flex flex-col items-center justify-center p-8 bg-[#E8D5B7]/95 backdrop-blur-sm animate-in fade-in duration-200">
            ...
            <h3 className="text-xl font-bold text-center mb-2" style={{ color: '#14532d' }}>Akun Sedang Aktif!</h3>
            <p className="text-center text-sm font-medium mb-6" style={{ color: '#d4af37' }}>
              Akun <strong className="text-[#14532d]">{username}</strong> saat ini sedang digunakan di perangkat lain. 
            </p>
            <button
            ...
            <button
              ...
              disabled={loading}
              className="w-full py-3 rounded-full font-bold transition-all bg-white text-[#14532d] hover:bg-gray-50 shadow-sm"
            >
              BATAL
            </button>
          </div>
        ...
        {/* Logo */}
        <div className="mb-4">
          <div className="w-24 h-24 rounded-full border-4 overflow-hidden flex items-center justify-center bg-black" style={{ borderColor: '#14532d' }}>
            <img src="/logo.jpeg" alt="Logo" className="w-full h-full object-cover" />
          </div>
        </div>

        {/* Title */}
        <h1 className="text-2xl font-bold mb-8 tracking-widest" style={{ color: '#14532d' }}>LOGIN KASIR</h1>
