    ...
    const fetchAudioUrl = async () => {
      try {
        ...
        if (url && url.startsWith('/uploads')) {
          url = Capacitor.isNativePlatform() ? `https://NewChapter.cloud${url}` : url;
        }
        if (url) {
        ...
        if (urlRem && urlRem.startsWith('/uploads')) {
          urlRem = Capacitor.isNativePlatform() ? `https://NewChapter.cloud${urlRem}` : urlRem;
        }
        if (urlRem) {
    ...
    <MobileLayout activeMenu="KDS">
      ...
      <div className="hidden lg:flex justify-between items-center px-6 xl:px-10 py-5 bg-white/80 backdrop-blur-md sticky top-0 z-10 border-b border-emerald-100/50 shadow-sm">
        ...
        <div className="flex items-center gap-4">
          ...
          <button
            ...
            className="px-3 md:px-4 py-2 rounded-full font-bold text-xs md:text-sm bg-[#14532d] hover:bg-[#4d3925] text-white transition-all shadow-sm flex items-center gap-1"
          >
            🔌 Printer
          </button>
          <button
            onClick={() => setShowAudioModal(true)}
            className="px-3 md:px-4 py-2 rounded-full font-bold text-xs md:text-sm bg-[#d4af37] hover:bg-[#14532d] text-white transition-all shadow-sm flex items-center gap-1"
          >
            🎵 Nada
          </button>
          <button
      ...
      <div className="flex-1 p-4 md:p-6 xl:p-10 overflow-auto scroll-smooth">
        ...
        <div className="lg:hidden mb-4 flex justify-between items-start">
          <div>
            <h1 className="text-xl font-black bg-clip-text text-transparent bg-gradient-to-r from-[#14532d] to-[#b8860b]">
              KDS - Kitchen Display
            </h1>
        ...
        <div className="flex flex-wrap items-center justify-between gap-2 mb-4">
          <div className="flex gap-2 overflow-x-auto pb-1">
            <button onClick={() => setKdsMode('semua')} className="px-4 py-2 rounded-full font-bold text-xs shadow-sm transition-all whitespace-nowrap" style={{ backgroundColor: kdsMode === 'semua' ? '#14532d' : '#fff', color: kdsMode === 'semua' ? '#fff' : '#14532d', border: '1px solid #14532d' }}>Semua</button>
            <button onClick={() => setKdsMode('dapur')} className="px-4 py-2 rounded-full font-bold text-xs shadow-sm transition-all whitespace-nowrap flex items-center gap-1" style={{ backgroundColor: kdsMode === 'dapur' ? '#14532d' : '#fff', color: kdsMode === 'dapur' ? '#fff' : '#14532d', border: '1px solid #14532d' }}><Utensils size={14}/> Dapur</button>
            <button onClick={() => setKdsMode('bar')} className="px-4 py-2 rounded-full font-bold text-xs shadow-sm transition-all whitespace-nowrap flex items-center gap-1" style={{ backgroundColor: kdsMode === 'bar' ? '#14532d' : '#fff', color: kdsMode === 'bar' ? '#fff' : '#14532d', border: '1px solid #14532d' }}><Coffee size={14}/> Bar</button>
          </div>
          <div className="flex items-center gap-2">
            ...
            <button onClick={requestPrinterPermission} className="lg:hidden px-3 py-2 bg-[#14532d] hover:bg-[#4d3925] text-white text-xs font-bold rounded-full flex items-center gap-1 transition-all shadow-sm">
              🔌 Printer
            </button>
            <button
              onClick={() => setShowAudioModal(true)}
              className="lg:hidden px-3 py-2 bg-[#d4af37] hover:bg-[#14532d] text-white text-xs font-bold rounded-full flex items-center gap-1 transition-all shadow-sm"
            >
              🎵 Nada
            </button>
            <button
        ...
        {loading ? (
          <div className="flex items-center justify-center py-24">
            <p style={{ color: '#d4af37' }}>Memuat pesanan...</p>
          </div>
        ) : pesananList.length === 0 ? (
          <div className="flex items-center justify-center py-24">
            <p style={{ color: '#d4af37' }}>Tidak ada pesanan aktif</p>
          </div>
        ) : (
          <div>
                ...
                >
                  ...
                  >
                    <div className="flex items-center gap-2 md:gap-3">
                      <div className="w-9 h-9 md:w-10 md:h-10 rounded-xl md:rounded-2xl flex items-center justify-center bg-emerald-100 text-emerald-700 shadow-sm border border-emerald-200 text-lg md:text-xl">
                        {pesanan.tipe === 'take-away' ? <ShoppingBag size={20} /> : <Utensils size={20} />}
                      </div>
                      <div className="flex flex-col">
                        <h2 className="text-base md:text-lg font-black bg-clip-text text-transparent bg-gradient-to-r from-[#14532d] to-[#b8860b]">
                    ...
                    >
                      ...
                      <div
                        ...
                        style={{
                          ...
                          color: item.status === 'pending' ? '#14532d' : '#fff'
                        }}
                      >
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="font-bold text-xs md:text-sm truncate" style={{ color: '#14532d' }}>
                          {item.nama_menu}
                        </p>
                        {item.catatan && (
                          <button 
                          ...
                          >
                            <div className="flex items-center gap-1">
                              <FileText size={12} className="text-emerald-700 flex-shrink-0" /> 
                              <span className="text-[10px] font-semibold text-emerald-700 truncate max-w-[150px] md:max-w-none">
                                {item.catatan}
                              </span>
                            </div>
                          </button>
                      ...
                      <div className="px-2 py-1 rounded-md bg-[#F5F0E8] border border-[#EDE0CC] flex-shrink-0">
                        <p className="font-black text-xs" style={{ color: '#14532d' }}>
                          {item.qty}x
                        </p>
                      </div>
        ...
        >
          ...
          >
            <div className="flex items-center gap-3 mb-4">
              <div className="w-10 h-10 rounded-xl bg-emerald-50 text-emerald-600 flex items-center justify-center border border-emerald-100 shadow-sm flex-shrink-0">
                <FileText size={20} />
              </div>
              <div className="min-w-0 flex-1">
                <h3 className="font-black text-lg bg-clip-text text-transparent bg-gradient-to-r from-[#14532d] to-[#b8860b] truncate">
                  Catatan Pesanan
                </h3>
            ...
            <div className="bg-[#FFFBEB] p-4 rounded-xl border border-[#FFE4A0] shadow-inner max-h-[40vh] overflow-y-auto">
              <p className="text-[#14532d] text-sm whitespace-pre-wrap leading-relaxed font-medium">
            ...
            <button 
              ...
              style={{ backgroundColor: '#14532d', boxShadow: '0 4px 14px rgba(99, 73, 48, 0.3)' }}
            >
              TUTUP
            </button>
          </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="border border-gray-200 p-4 rounded-xl">
                  ...
                  <input 
                    ...
                    className="w-full text-xs text-gray-500 file:mr-2 file:py-1 file:px-3 file:rounded-lg file:border-0 file:text-xs file:font-semibold file:bg-emerald-50 file:text-emerald-700 hover:file:bg-emerald-100 mb-4"
                  />
        ...
        >
          ...
          >
            ...
            <div className="p-2 mb-2">
              <p className="text-[#14532d] text-sm font-medium">
