...
export default function KasirPOS() {
  ...
  return (
    <MobileLayout activeMenu="Kasir (POS)" overflowClass="overflow-hidden flex flex-col">
      ...
      <div className="hidden lg:flex justify-between items-center px-8 py-4 shadow-sm" style={{ backgroundColor: '#EDE0CC' }}>
        ...
        <div className="flex items-center gap-3">
          <div className="text-right">
            <p className="text-sm font-bold" style={{ color: '#14532d' }}>Kasir</p>
            <p className="text-sm" style={{ color: '#d4af37' }}>{user?.username}</p>
          </div>
          <div className="w-10 h-10 rounded-full flex items-center justify-center text-white font-bold text-lg" style={{ backgroundColor: '#14532d' }}>
      ...
      {loading ? (
        <div className="flex-1 flex items-center justify-center"><p style={{ color: '#14532d' }}>Memuat data...</p></div>
      ) : (
        <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
          ...
          <div className="flex-1 flex flex-col p-3 md:p-6 overflow-hidden">
            ...
            <div className="flex flex-wrap gap-2 md:gap-3 mb-3 md:mb-4">
              <div className="flex-1 min-w-[150px] relative">
                <input type="text" placeholder="Cari Menu..." value={search} onChange={e => setSearch(e.target.value)}
                  className="w-full px-4 py-2.5 md:py-3 rounded-full text-sm focus:outline-none" style={{ backgroundColor: '#EDE0CC', color: '#14532d', border: '1.5px solid #C4A882' }} />
                <span className="absolute right-4 top-1/2 -translate-y-1/2" style={{ color: '#d4af37' }}><Search size={16} /></span>
              </div>
              <div className="flex rounded-full overflow-hidden" style={{ border: '1.5px solid #C4A882' }}>
                <button onClick={() => setTipeOrder('dine-in')} className="px-3 md:px-4 py-2.5 md:py-3 text-xs md:text-sm font-medium transition-all"
                  style={{ backgroundColor: tipeOrder === 'dine-in' ? '#14532d' : '#EDE0CC', color: tipeOrder === 'dine-in' ? '#fff' : '#14532d' }}><span className="flex items-center gap-1.5"><Utensils size={14} /> Dine In</span></button>
                <button onClick={() => setTipeOrder('take-away')} className="px-3 md:px-4 py-2.5 md:py-3 text-xs md:text-sm font-medium transition-all"
                  style={{ backgroundColor: tipeOrder === 'take-away' ? '#14532d' : '#EDE0CC', color: tipeOrder === 'take-away' ? '#fff' : '#14532d' }}><span className="flex items-center gap-1.5"><ShoppingBag size={14} /> TA</span></button>
              </div>
              ...
              <div className="lg:hidden flex gap-1.5 items-center">
                <select
                  ...
                  style={{ backgroundColor: '#EDE0CC', color: '#14532d', borderColor: '#C4A882' }}
                >
                ...
                {printMethod === 'thermal' && (
                  <button onClick={() => setShowPrinterModal(true)} className="px-3 py-2 bg-[#14532d] hover:bg-[#4d3925] text-white text-xs font-bold rounded-full flex items-center gap-1 transition-all">
                    🔌 Pair
                  </button>
                )}
              </div>
            ...
            <div className="flex gap-2 md:gap-3 mb-3 md:mb-5 overflow-x-auto pb-1">
              <button onClick={() => setKategori('semua')} className="px-4 md:px-8 py-2 rounded-full font-medium text-xs md:text-sm transition-all whitespace-nowrap flex-shrink-0"
                style={{ backgroundColor: kategori === 'semua' ? '#fff' : 'transparent', color: '#14532d', border: kategori === 'semua' ? '2px solid #14532d' : '2px solid #C4A882' }}>Semua</button>
              {kategoriList.map(k => (
                <button key={k.id} onClick={() => setKategori(k.nama)} className="px-4 md:px-8 py-2 rounded-full font-medium text-xs md:text-sm transition-all whitespace-nowrap flex-shrink-0"
                  style={{ backgroundColor: kategori === k.nama ? '#fff' : 'transparent', color: '#14532d', border: kategori === k.nama ? '2px solid #14532d' : '2px solid #C4A882' }}>{k.nama}</button>
              ))}
            </div>
            ...
            <div className="flex-1 overflow-y-auto">
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 md:gap-4">
                {filteredMenu.length === 0 ? (
                  <p className="col-span-full text-center py-12 text-sm" style={{ color: '#d4af37' }}>Tidak ada menu</p>
                ) : filteredMenu.map(menu => {
                  const qty = getQty(menu.id)
                  return (
                    <div key={menu.id} className="rounded-2xl overflow-hidden flex flex-col shadow-sm"
                      style={{ backgroundColor: '#EDE0CC', border: qty > 0 ? '2px solid #14532d' : '2px solid transparent' }}>
                      {/* Menu Image */}
                      ...
                      <div className="p-2 md:p-3 flex flex-col items-center flex-1 justify-between w-full">
                        <p className="text-xs font-bold text-center mb-1 leading-snug min-h-[2rem] flex items-center justify-center" style={{ color: '#14532d' }}>{menu.nama}</p>
                        {menu.deskripsi && menu.deskripsi !== '-' && menu.deskripsi.trim() !== '' && (
                          ...
                          >
                            ...
                            {expandedDescId !== menu.id && menu.deskripsi.length > 40 && (
                              <p className="text-[8px] text-center text-[#d4af37] mt-0.5 font-bold hover:underline">Baca selengkapnya...</p>
                            )}
                          </div>
                        )}
                        {(() => {
                          ...
                          return (
                            <>
                              {activePromo ? (
                                <div className="flex flex-col items-center gap-0.5 mb-1 w-full">
                                  ...
                                  {activePromo.mulai_jam && activePromo.selesai_jam && (
                                    <span className="text-[9px] font-bold px-1.5 py-0.5 rounded-full flex items-center gap-0.5 bg-emerald-100 text-emerald-700">
                                      ⏰ {activePromo.mulai_jam}-{activePromo.selesai_jam}
                                    </span>
                                  )}
                                </div>
                              ...
                              {activePromo ? (
                                ...
                                <p className="text-xs mb-2" style={{ color: '#d4af37' }}>Rp {Number(menu.harga).toLocaleString('id-ID')}</p>
                              )}
                            </>
                          )
                        })()}
                        {userCanEdit('pos') && (
                          qty === 0 ? (
                            <button onClick={() => tambahItem(menu)} className="w-full py-1.5 rounded-full text-xs font-bold transition-all" style={{ backgroundColor: '#14532d', color: '#fff' }}>+ Tambah</button>
                          ) : (
                            <div className="flex items-center gap-2">
                              ...
                              <span className="font-bold text-sm" style={{ color: '#14532d' }}>{qty}</span>
          ...
          {totalItems > 0 && (
            <button onClick={() => setShowOrderPanel(true)}
              className="lg:hidden fixed bottom-6 right-6 z-30 w-14 h-14 rounded-full bg-[#14532d] text-white shadow-xl flex items-center justify-center text-xl">
          ...
          <div className={`fixed right-0 top-0 bottom-0 z-50 lg:static lg:z-auto w-[320px] md:w-80 max-w-full flex flex-col shadow-xl transition-transform duration-300 ${showOrderPanel ? 'translate-x-0' : 'translate-x-full lg:translate-x-0'}`} style={{ backgroundColor: '#fff' }}>
            <div className="p-4 md:p-5 border-b flex justify-between items-center" style={{ borderColor: '#EDE0CC' }}>
              <h2 className="text-base md:text-lg font-bold" style={{ color: '#14532d' }}>
                Order {tipeOrder === 'take-away' ? '(TA)' : ''}
              </h2>
              <button onClick={() => setShowOrderPanel(false)} className="lg:hidden text-[#d4af37] text-xl"><X size={20} /></button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3">
              ...
              <div className="p-2.5 rounded-xl space-y-2 border" style={{ backgroundColor: '#FDFBF7', borderColor: '#EDE0CC' }}>
                <p className="text-xs font-bold uppercase tracking-wider" style={{ color: '#d4af37' }}>Informasi Pelanggan</p>
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    <select value={tipePelanggan} onChange={e => handleTipePelangganChange(e.target.value)}
                      className="w-full text-[11px] px-2.5 py-1.5 rounded-lg font-bold focus:outline-none cursor-pointer" style={{ backgroundColor: '#F5F0E8', color: '#14532d', border: '1px solid #C4A882' }}>
                  ...
                  {tipePelanggan === 'Member' ? (
                    <div className={tipeOrder === 'dine-in' ? 'col-span-1 flex gap-1' : 'col-span-2 flex gap-2'}>
                      <input type="text" placeholder="No HP/WA" value={nomorHp} onChange={e => { setNomorHp(e.target.value); setMemberInfo(null); }}
                        className="flex-1 w-full min-w-0 text-[11px] px-2.5 py-1.5 rounded-lg focus:outline-none" style={{ backgroundColor: '#F5F0E8', color: '#14532d', border: '1px solid #C4A882' }} />
                      <button onClick={checkMember} className="px-2 shrink-0 rounded-lg text-white font-bold text-[11px] bg-[#14532d] hover:bg-[#0f4022]">Cek</button>
                    </div>
                  ) : (
                    <div className={tipeOrder === 'dine-in' ? 'col-span-1' : 'col-span-2'}>
                      <input type="text" placeholder="Nama Pelanggan" value={namaPelanggan} onChange={e => setNamaPelanggan(e.target.value)}
                        className="w-full text-[11px] px-2.5 py-1.5 rounded-lg focus:outline-none" style={{ backgroundColor: '#F5F0E8', color: '#14532d', border: '1px solid #C4A882' }} />
                    </div>
                  )}
                  {tipeOrder === 'dine-in' && (
                    <div className="relative">
                      <button
                        ...
                        style={{ backgroundColor: '#F5F0E8', color: '#14532d', border: '1px solid #C4A882', fontWeight: 'bold' }}
                      >
                        ...
                        <span className="text-[8px] text-[#14532d]/60 shrink-0 ml-1">▼</span>
                      </button>
                ...
                {memberInfo && tipePelanggan === 'Member' && (
                  <div className="mt-2 p-3 rounded-lg border flex items-center justify-between shadow-sm" style={{ backgroundColor: '#F9F5F0', borderColor: '#d4af37' }}>
                    <div>
                      <p className="text-[12px] font-black" style={{ color: '#14532d' }}>{memberInfo.nama_panggilan || memberInfo.nama}</p>
                      <p className="text-[10px] font-bold mt-0.5" style={{ color: '#d4af37' }}>Poin Tersedia: {memberInfo.point}</p>
                    </div>
                    <div className="w-8 h-8 rounded-full flex items-center justify-center bg-[#14532d]/10 border border-[#d4af37]/50">
              ...
              {order.length === 0 ? (
                <p className="text-center text-sm py-8" style={{ color: '#d4af37' }}>Belum ada item</p>
              ) : order.map(o => (
                <div key={o.menu_id} className="flex gap-3 pb-3 border-b" style={{ borderColor: '#EDE0CC' }}>
                  <div className="flex-1">
                    <div className="flex justify-between items-start">
                      <p className="text-sm font-medium" style={{ color: '#14532d' }}>{o.nama}</p>
                      <p className="text-sm font-bold" style={{ color: '#14532d' }}>{(o.harga * o.qty).toFixed(1)}</p>
                    </div>
                    <div className="flex items-center gap-2 mt-1">
                      <p className="text-xs" style={{ color: '#d4af37' }}>{o.harga.toFixed(1)}</p>
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <span className="text-xs font-bold" style={{ color: '#14532d' }}>{o.qty}</span>
                    ...
                    <input type="text" placeholder="Catatan..." value={o.catatan} onChange={e => updateCatatan(o.menu_id, e.target.value)}
                      className="mt-1 w-full text-xs px-2 py-1 rounded focus:outline-none" style={{ backgroundColor: '#F5F0E8', color: '#14532d' }} />
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="p-4 border-t" style={{ borderColor: '#EDE0CC' }}>
              {Number(pointUsed) > 0 || Number(discountValue) > 0 ? (
                <>
                  <div className="flex justify-between text-xs mb-1"><span style={{ color: '#d4af37' }}>Subtotal</span><span style={{ color: '#14532d' }}>Rp {subtotal.toLocaleString('id-ID')}</span></div>
                  {Number(discountValue) > 0 && <div className="flex justify-between text-xs mb-1"><span style={{ color: '#d4af37' }}>Diskon ({discountName})</span><span className="text-red-500">-Rp {Number(discountValue).toLocaleString('id-ID')}</span></div>}
                  {Number(pointUsed) > 0 && <div className="flex justify-between text-xs mb-1"><span style={{ color: '#d4af37' }}>Tukar Poin</span><span className="text-red-500">-Rp {Number(pointUsed).toLocaleString('id-ID')}</span></div>}
                </>
              ) : null}
              <div className="flex justify-between text-sm mb-2 mt-1"><span className="font-bold" style={{ color: '#14532d' }}>TOTAL Tagihan</span><span className="font-bold" style={{ color: '#14532d' }}>Rp {total.toLocaleString('id-ID')}</span></div>
              ...
              <div className="flex gap-2 mb-3">
                <select value={metodeBayar} onChange={e => setMetodeBayar(e.target.value)} className="flex-1 px-3 py-2 rounded-lg text-sm focus:outline-none" style={{ backgroundColor: '#F5F0E8', color: '#14532d', border: '1px solid #C4A882' }}>
                ...
                {metodeBayar === 'Tunai' && (
                  <input type="text" placeholder="Jumlah bayar" value={jumlahBayar} onChange={e => setJumlahBayar(formatRibuan(e.target.value))}
                    className="flex-1 px-3 py-2 rounded-lg text-sm focus:outline-none" style={{ backgroundColor: '#F5F0E8', color: '#14532d', border: '1px solid #C4A882' }} />
                )}
              </div>
              {metodeBayar === 'Tunai' && <p className="text-sm mb-3 text-right" style={{ color: '#14532d' }}>Kembali: Rp {kembali.toLocaleString('id-ID')}</p>}
      ...
      {showSpinModal && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl p-6 max-w-md w-full shadow-2xl border-2 border-[#C4A882] max-h-[90vh] overflow-y-auto">
            <div className="mb-4">
              <h3 className="font-extrabold text-lg text-[#14532d]">Belanja Rp {total.toLocaleString('id-ID')} (Kelipatan 100k)!</h3>
            ...
            <div className="bg-[#F5F0E8] rounded-2xl p-4 mb-4 border border-[#C4A882]/40">
              <p className="text-xs font-semibold text-[#14532d] mb-3">
              ...
              <div className="space-y-3">
                {selectedSpinPrizes.map((prize, idx) => (
                  <div key={idx}>
                    <label className="block text-xs font-bold text-[#14532d] mb-1">Hadiah Putaran ke-{idx + 1}:</label>
                    <select 
                      ...
                      className="w-full p-2.5 rounded-xl border-2 border-[#C4A882] bg-white font-bold text-xs text-[#14532d] focus:outline-none"
                    >
        ...
        <div className="fixed inset-0 bg-black/50 z-[100] flex items-center justify-center p-4">
          <div className="bg-[#FDFBF7] rounded-2xl p-6 w-full max-w-md max-h-[90vh] overflow-y-auto border-2 border-[#C4A882]">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-lg font-bold text-[#14532d]">Pengaturan Multi-Printer</h2>
              <button onClick={() => setShowPrinterModal(false)} className="text-[#d4af37] hover:text-[#14532d] font-bold text-xl">✕</button>
            </div>
            
            {!window.bluetoothSerial ? (
              <div className="text-sm text-[#d4af37] mb-4 bg-[#EDE0CC] p-4 rounded-xl border border-[#C4A882]/40">
                <p className="font-bold text-[#14532d] mb-2">🌐 Mode Desktop (Web Serial)</p>
                ...
                <div className="mt-4">
                  <button onClick={() => { requestPrinterPermission(); setShowPrinterModal(false); }} className="w-full py-2.5 bg-[#14532d] text-white font-bold rounded-xl flex justify-center items-center gap-2">
              ...
              <div className="space-y-4">
                <div className="flex justify-end">
                  <button 
                    ...
                    className="px-3 py-1.5 bg-[#EDE0CC] hover:bg-[#C4A882] text-xs font-bold rounded-lg text-[#14532d] flex items-center gap-1 transition-all"
                  >
                  ...
                  {['kasir', 'dapur', 'bar'].map(role => (
                    <div key={role} className="border border-[#C4A882]/40 p-3 rounded-xl bg-white shadow-sm">
                      ...
                      <select 
                        ...
                        className="w-full p-2.5 rounded-lg bg-[#F5F0E8] border border-[#C4A882]/40 text-sm font-bold text-[#14532d] focus:outline-none"
                      >
                ...
                <button 
                  ...
                  className="w-full mt-2 py-3 bg-[#14532d] text-white font-bold rounded-xl shadow-md active:scale-95 transition-all flex justify-center items-center gap-2"
                >
                  💾 Simpan Pengaturan
                </button>
              </div>
      ...
      {showMemberModal && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-[#FDFBF7] rounded-3xl w-full max-w-sm overflow-hidden shadow-2xl border border-[#C4A882]/30">
            <div className="bg-[#14532d] p-4 text-center">
            ...
            <div className="p-5 space-y-4">
              <div>
                <label className="block text-xs font-bold text-[#d4af37] mb-1">Nomor HP / WA *</label>
              ...
              <div>
                <label className="block text-xs font-bold text-[#d4af37] mb-1">Nama (Sapaan/Struk) *</label>
              ...
              <div>
                <label className="block text-xs font-bold text-[#d4af37] mb-1">Tanggal Lahir (Opsional)</label>
              ...
              <div className="pt-2 flex gap-3">
                <button onClick={() => setShowMemberModal(false)} className="flex-1 py-3 bg-[#EDE0CC] text-[#14532d] font-bold rounded-xl">Batal</button>
                <button onClick={registerMember} className="flex-1 py-3 bg-[#14532d] text-white font-bold rounded-xl">Daftar</button>
              </div>
            </div>
          </div>
        </div>
