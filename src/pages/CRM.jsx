...
export default function CRM() {
  ...
  const [broadcastMessage, setBroadcastMessage] = useState('Halo [Nama], kami kangen nih! Ada diskon 20% khusus buat kamu yang balik lagi ke New Chapter minggu ini.')
  ...
  useEffect(() => {
    ...
    const socketUrl = Capacitor.isNativePlatform() ? 'https://NewChapter.cloud' : (import.meta.env.VITE_API_URL || '')
      ...
      <div className="p-4 md:p-8 bg-[#F9F5F0] min-h-full pb-24">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
          <div>
            <h1 className="text-2xl font-black text-[#442D1D] flex items-center gap-2">
              <Users className="text-[#14532d]" size={28} /> CRM & Pelanggan
            </h1>
            <p className="text-[#d4af37] mt-1 text-sm font-medium">Data kunjungan dan WhatsApp Broadcast.</p>
          </div>
          ...
          {canEdit('crm') && selectedPhones.length > 0 && (
            <div className="flex items-center gap-2 bg-white px-4 py-2 rounded-xl shadow-sm border border-stone-200">
              <span className="text-sm font-bold text-[#14532d] mr-2">
          ...
          <div className="mb-6 bg-white p-5 rounded-2xl shadow-sm border border-stone-200 flex flex-col md:flex-row items-center justify-between gap-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <div className="flex items-center gap-2 mt-1">
                  <div className={`w-2.5 h-2.5 rounded-full ${waStatus === 'CONNECTED' ? 'bg-[#21B214] animate-pulse' : waStatus === 'QR_READY' ? 'bg-emerald-500 animate-pulse' : 'bg-stone-300'}`}></div>
            ...
            <div className="flex flex-col sm:flex-row items-center gap-4">
              {waStatus === 'QR_READY' && waQr && (
                <div className="flex flex-col items-center gap-3 bg-emerald-50 p-4 rounded-xl border border-emerald-100">
                  <img src={waQr} alt="WhatsApp QR Code" className="w-48 h-48 md:w-64 md:h-64 rounded bg-white p-2 shadow-sm" />
                  <p className="text-xs text-emerald-800 font-bold text-center max-w-[200px]">
        ...
        <div className="flex flex-col md:flex-row gap-4 mb-6 justify-between items-start md:items-center">
          {viewMode === 'bulanan' ? (
                ...
                <button
                  ...
                  className={`px-5 py-2 rounded-full font-bold text-xs whitespace-nowrap transition-all ${
                    filterVisit === f.val 
                      ...
                      : 'text-[#d4af37] hover:bg-stone-100'
                  }`}
                >
                  {f.label}
                </button>
            ...
            <div className="text-xs font-bold text-[#d4af37] bg-[#FFF5E5] px-4 py-2 rounded-full border border-[#EDE0CC]">
          ...
          <div className="relative w-full md:w-64">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2 bg-white border border-stone-200 rounded-full focus:outline-none focus:border-[#14532d] text-sm text-[#442D1D]"
            />
          </div>
        </div>
                    ...
                    <tr className="bg-stone-50 border-b border-stone-200">
                      {canEdit('crm') && (
                        <th className="py-3 px-4 w-12 text-center">
                          <button onClick={toggleSelectAll} className="text-stone-400 hover:text-[#14532d]">
                            {filteredMembers.length > 0 && filteredMembers.filter(m => m.no_hp).every(m => selectedPhones.includes(m.no_hp)) 
                              ? <CheckSquare size={20} className="text-[#14532d]" /> 
                              : <Square size={20} />}
                          </button>
                        </th>
                      )}
                ...
                <tbody className="divide-y divide-stone-100">
                  {viewMode === 'member' ? (
                    filteredMembers.map((m, idx) => {
                      ...
                      return (
                      <tr key={idx} className={`hover:bg-stone-50 transition-colors ${isSelected ? 'bg-emerald-50/50' : ''}`}>
                        {canEdit('crm') && (
                          <td className="py-3 px-4 text-center">
                            {m.no_hp ? (
                              <button onClick={() => toggleSelect(m.no_hp)} className="text-stone-400">
                                {isSelected ? <CheckSquare size={20} className="text-[#14532d]" /> : <Square size={20} />}
                              </button>
                            ) : (
                        ...
                        <td className="py-3 px-4">
                          <div className="flex items-center gap-3">
                            <div className="w-10 h-10 rounded-full bg-[#FFF5E5] flex items-center justify-center text-[#14532d] font-bold">
                        ...
                        <td className="py-3 px-4 text-center">
                          <div className="flex items-center justify-center gap-1">
                            <button 
                              ...
                              title="Riwayat Poin"
                              className="bg-[#14532d]/10 text-[#14532d] hover:bg-[#14532d]/20 p-2 rounded-lg font-bold transition-colors"
                            >
                              <Activity size={16} />
                            </button>
                            {canEdit('crm') && (
                    ...
                    filteredCustomers.map((cust, idx) => {
                      ...
                      return (
                        <tr 
                          key={idx} 
                          className={`hover:bg-stone-50 transition-colors ${isSelected ? 'bg-emerald-50/50' : ''}`}
                        >
                          {canEdit('crm') && (
                            <td className="py-3 px-4 text-center">
                              {cust.no_telepon_wa ? (
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                <p className="font-bold text-sm text-stone-700 flex items-center gap-1">
                                  {cust.nama_pelanggan}
                                  {isLoyal && <Star size={14} className="text-emerald-500 fill-emerald-500" title="Pelanggan Loyal" />}
                                </p>
                          ...
                          <td className="py-3 px-4 text-center">
                            <span className={`px-3 py-1 rounded-full text-xs font-bold ${
                              isLoyal ? 'bg-emerald-100 text-emerald-700' : 'bg-stone-100 text-stone-600'
                            }`}>
          ...
          <div className="bg-white rounded-3xl max-w-lg w-full overflow-hidden shadow-2xl">
            ...
            <div className="p-6">
              <div className="mb-4">
                ...
                <div className="bg-stone-50 border border-stone-200 rounded-xl p-3 text-sm font-bold text-[#14532d]">
              ...
              <div>
                ...
                <p className="text-xs text-stone-500 mt-2">
                  Gunakan <strong className="text-[#14532d]">[Nama]</strong> agar sistem otomatis mengubahnya menjadi nama pelanggan.
                </p>
              </div>
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-[#FDFBF7] rounded-3xl w-full max-w-sm overflow-hidden shadow-2xl border border-[#C4A882]/30">
            ...
            <div className="p-5 space-y-4">
              ...
              <div>
                <label className="block text-xs font-bold text-[#d4af37] mb-1">Tanggal Lahir</label>
              ...
              <div className="pt-2 flex gap-3">
                <button onClick={() => setEditMemberModal({ show: false, data: {} })} className="flex-1 py-3 bg-[#EDE0CC] text-[#14532d] font-bold rounded-xl">Batal</button>
      ...
      {deleteMemberModal.show && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-[#FDFBF7] rounded-3xl w-full max-w-sm overflow-hidden shadow-2xl border border-[#C4A882]/30 p-5 text-center">
            ...
            <p className="text-[#d4af37] text-sm mb-6">Anda yakin ingin menghapus <strong>{deleteMemberModal.nama}</strong> dari keanggotaan? Seluruh poinnya akan hilang.</p>
            <div className="flex gap-3">
              <button onClick={() => setDeleteMemberModal({ show: false, id: null, nama: '' })} className="flex-1 py-3 bg-[#EDE0CC] text-[#14532d] font-bold rounded-xl">Batal</button>
