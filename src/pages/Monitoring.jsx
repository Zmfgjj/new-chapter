        ...
        <div className="p-6 md:p-8 flex flex-col md:flex-row md:items-center md:justify-between gap-4 border-b border-[#EDE0CC] bg-white">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="flex items-center gap-2 px-4 py-2.5 rounded-xl font-semibold text-[#14532d] bg-[#F5F0E8] hover:bg-[#EDE0CC] border border-[#C4A882]/30 transition-all duration-200"
            >
        ...
        <div className="flex-1 overflow-y-auto p-6 md:p-8">
          ...
          {activeTab === 'logs' && (
            <div className="space-y-6">
              ...
              <div className="bg-white p-4 rounded-2xl border border-[#EDE0CC] flex flex-wrap items-center gap-3">
                <span className="text-xs font-bold text-[#14532d] uppercase">Filter Tipe Aksi:</span>
                <button
                  onClick={() => setFilterAction('')}
                  className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${
                    filterAction === '' ? 'bg-[#14532d] text-white' : 'bg-[#F5F0E8] text-[#14532d] hover:bg-[#EDE0CC]'
                  }`}
                >
                  Semua Aksi
                </button>
              ...
              {loadingLogs ? (
                <div className="flex flex-col items-center justify-center py-20">
                  <div className="w-10 h-10 border-4 border-[#14532d]/20 border-t-[#14532d] rounded-full animate-spin mb-4"></div>
                    ...
                    >
                      <div className="space-y-1.5 flex-1">
                        ...
                        <p className="text-xs text-gray-500">
                          Pelaku: <strong className="text-[#14532d]">{log.username}</strong> (ID: {log.user_id || 'System'})
                        </p>
                      </div>
                      ...
                      {log.action_type === 'DELETE' && log.has_backup && (
                        <div>
                          <button
                            ...
                            className="w-full md:w-auto px-4 py-2.5 rounded-xl text-xs font-bold text-white bg-gradient-to-r from-emerald-600 to-emerald-700 hover:from-emerald-700 hover:to-emerald-800 transition-all flex items-center justify-center gap-1.5 shadow-md shadow-emerald-600/10 active:scale-95 disabled:opacity-50"
                          >
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <div className="bg-white p-6 rounded-3xl border border-[#EDE0CC] shadow-sm flex flex-col justify-between">
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <Cpu className="text-[#14532d]" size={22} />
                    </div>
                    <div className="space-y-3">
                      <p className="text-xs text-gray-400 font-semibold line-clamp-1">{sysStatus.cpu.model}</p>
                      <div className="flex justify-between items-end">
                        <span className="text-3xl font-black text-[#14532d]">{sysStatus.cpu.cores} Cores</span>
                      ...
                      <div className="w-full bg-[#F5F0E8] h-2 rounded-full overflow-hidden">
                        <div className="bg-[#14532d] h-full" style={{ width: '40%' }}></div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-white p-6 rounded-3xl border border-[#EDE0CC] shadow-sm flex flex-col justify-between">
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex justify-between items-end">
                        <span className="text-3xl font-black text-[#14532d]">{sysStatus.memory.percentage}%</span>
                        <span className="text-xs font-bold text-gray-400">Bebas: {formatBytes(sysStatus.memory.free)}</span>
                      </div>
                      <div className="w-full bg-[#F5F0E8] h-2 rounded-full overflow-hidden">
                        <div className="bg-emerald-600 h-full" style={{ width: `${sysStatus.memory.percentage}%` }}></div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-white p-6 rounded-3xl border border-[#EDE0CC] shadow-sm md:col-span-3 grid grid-cols-1 md:grid-cols-4 gap-6">
                    <div className="flex items-center gap-4">
                      <div className="w-12 h-12 rounded-2xl bg-emerald-50 text-[#d4af37] flex items-center justify-center shrink-0">
                        <Clock size={24} />
                      </div>
                      <div>
                    ...
                    <div className="flex items-center gap-4">
                      <div className="w-12 h-12 rounded-2xl bg-[#F5F0E8] text-[#14532d] flex items-center justify-center shrink-0">
                  ...
                  <div className="bg-white p-6 rounded-3xl border border-[#EDE0CC] shadow-sm md:col-span-3">
                    <h3 className="font-bold text-stone-700 mb-4 flex items-center gap-2">
                      <Cpu className="text-[#14532d]" size={20} /> Top Proses Memori (Server)
                    </h3>
                        ...
                        <tbody>
                          {sysStatus.processes?.map((p, i) => (
                            <tr key={i} className="border-b border-gray-50 hover:bg-[#F9F5F0]">
                              ...
                              <td className="py-2 text-emerald-600 font-bold">{p.cpu}%</td>
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-4 gap-4">
                      <div>
                        <h3 className="font-bold text-stone-700 flex items-center gap-2">
                          <HardDrive className="text-[#14532d]" size={20} /> Manajemen Penyimpanan
                        </h3>
                        ...
                        return (
                          <div key={i} className="border border-gray-100 rounded-2xl p-4 bg-[#F9F5F0] flex flex-col justify-between gap-3">
                            <div>
                              ...
                              <p className="text-2xl font-black text-[#14532d]">{folder.size}</p>
                              <p className="text-[10px] text-gray-500 font-mono mt-1 break-all">{folder.path}</p>
                            </div>
                            <button
                              onClick={() => handleClearCache(targetKey)}
                              disabled={clearLoading !== false}
                              className="w-full py-2 bg-white text-[#14532d] hover:bg-stone-50 border border-[#EDE0CC] font-bold rounded-lg text-xs transition-colors"
                            >
                  ...
                  {/* Server Control Card */}
                  <div className="bg-white p-6 rounded-3xl border border-emerald-200/80 shadow-sm md:col-span-3 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-gradient-to-r from-emerald-50/40 to-white">
                    <div>
                      <h3 className="font-bold text-stone-800 flex items-center gap-2 text-base">
                        <Server className="text-emerald-600" size={20} /> Kontrol &amp; Reboot Server (PM2 &amp; VPS)
                      </h3>
                    ...
                    <div className="flex flex-wrap gap-2 shrink-0">
                      <button
                        ...
                        className="px-4 py-3 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-xl text-xs transition-all shadow-md flex items-center gap-2 disabled:opacity-50"
                      >
          ...
          {activeTab === 'connectivity' && (
            <div className="space-y-6">
              ...
              <div className="bg-emerald-50/50 border border-emerald-200/60 p-5 rounded-3xl flex gap-4">
                <AlertCircle className="text-emerald-600 shrink-0 mt-0.5" size={24} />
                <div className="space-y-1">
                  <h4 className="font-bold text-[#14532d] text-sm">💡 Mengapa pesanan masuk Histori tapi tidak muncul di Dapur (KDS)?</h4>
              ...
              <div className="bg-white p-6 md:p-8 rounded-3xl border border-[#EDE0CC] shadow-sm space-y-6">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-[#EDE0CC] pb-5">
                  ...
                  {offlineQueue.length > 0 && (
                    <button
                      ...
                      className="px-5 py-2.5 bg-[#14532d] hover:bg-[#4d3925] text-white text-xs font-bold rounded-xl shadow-md transition-all flex items-center justify-center gap-2 disabled:opacity-50"
                    >
                ...
                {loadingOffline ? (
                  <div className="flex flex-col items-center justify-center py-10">
                    <div className="w-8 h-8 border-3 border-[#14532d]/20 border-t-[#14532d] rounded-full animate-spin mb-3"></div>
                  ...
                  <div className="space-y-4 max-h-[400px] overflow-y-auto pr-1">
                    {offlineQueue.map((order, index) => (
                      <div key={index} className="p-4 bg-emerald-50/20 hover:bg-emerald-50/40 border border-emerald-100 rounded-2xl transition-all space-y-3">
